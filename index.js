...
bot.on("text",  async (ctx) => {
   ...
   //console.log(ctx.chat.id + " " + ctx.message.text);
    ...
    await ctx.replyWithHTML("Такої команди немає. Тицяй на кнопки 🙂", {protect_content: true,  ...Markup.keyboard(constans.MAIN_MENU_ARRAY).resize().oneTime()})
...
async function startBot(ctx) {
    return ctx.replyWithHTML(constans.GREETING, {protect_content: true,  ...Markup.keyboard(constans.MAIN_MENU_ARRAY).resize().oneTime()})
    //return ctx.replyWithHTML("<b>Бот наразі знаходиться у розробці.</b> \n\nТакож було створено канал https://t.me/+uA00yl9KweQ1MTQy  \nЛаскаво просимо!", Markup.removeKeyboard())
}
